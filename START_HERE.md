...
3. If durable state matters, determine the scope before retrieval:
   - **global / cross-project** -> load root `CURRENT.md` and `DECISIONS.md`;
...
4. explicit active entries in `WORKING_STYLE.md` when they apply to how the work is performed;
5. historical/superseded repository state;
6. chat recollection or model inference.

Do not let a working-style preference override a current project decision or an explicit user instruction.
...
4. **WORKING STYLE** -> `WORKING_STYLE.md` only for stable, collaboration-relevant preferences about how the user and ChatGPT work together.
5. **PROJECT** -> route project-specific current state or decisions through `PROJECTS.md` to the appropriate project folder.
6. **NOT-V1 / no legitimate home** -> do not invent a file. Tell the user the material does not fit the current structure and treat recurrence as a growth signal.
...
- the work is expected to span multiple sessions and has its own objective, current state, constraints, or decisions;
- project-specific durable material would otherwise begin accumulating in root `CURRENT.md` or `DECISIONS.md`;
...
Routing is clear; global files remain cross-project; project-specific state lives behind project front doors; working-style entries are compact; complete required retrieval remains easy.
...
When status is Watch, recommend the smallest structural correction first: register/split a project, compact state, supersede stale entries, or add one justified routed source of record.
...
- active durable state is too large to load or inspect reliably even after project scoping and compaction;
